...
public class LineItemsTable extends BaseWebElement implements EditableTable, ReadableTable, CheckableTable {
    ...
    private static final String ALL_ROWS_XPATH = "//tr[contains(@class,'x-grid-row') and not(contains(@class,'x-grid-row-summary x-grid-row-total'))]";
    ...
    private static final String OPEN_XPATH = "//*[@class='ibeExternalLink']";
    ...
    public void setCellValue(String value, String columnName, Map<String, String> rowFilter) {
        ...
        } else if (columnName.equals("Start") | (columnName.equals("End")) | (columnName.equals("End of Delivery"))) {
...
//            $x(cellXpath + "//div//input").sendKeys(Keys.ARROW_DOWN);
            ...
            Selenide.sleep(700);
            $x("//div[@data-ref='listWrap']//ul//*[text() = '" + value + "']").click();
            Selenide.sleep(700);
        ...
        }else if (columnName.equalsIgnoreCase("Cost Allocation")){
            setCostAllocationCellValue(cellXpath, value);
        }
        else {
    ...
    private void setPhaseCellValue(String cellXpath, String value) {
        ...
        $x(cellXpath + "//div//input").sendKeys(Keys.ARROW_DOWN);
        String listItemXpath = "//div[@data-ref='listWrap']//ul[@aria-hidden='false']//li[contains(text(), '" + value + "')][1]";
    ...
    private void setCostAllocationCellValue(String cellXpath, String value) {
        ...
        $x(cellXpath + "//div//input").clear();
        $x(cellXpath + "//div").scrollIntoView(true).click();
        $x(cellXpath + "//div//input").sendKeys(values[0]);
        Selenide.sleep(700);
        String listItemXpath = "//div[@data-ref='listWrap']//ul[@aria-hidden='false']//li[@role='option']//*[text()='"+values[0]+"']/ancestor::li[@role='option']";
        $x(listItemXpath).shouldBe(visible, Duration.ofSeconds(5));
        $x(listItemXpath).hover().click();
        $x(cellXpath + "//div//input").sendKeys(Keys.ENTER);
        $(String.valueOf(toSelenideElement().getText().equals(value)));
        AllureUtils.saveScreenshot();
    }
    ...
    public int getRowIndex(Map<String, String> filter) {
        String rowXPath = appendXPathByFilter(ALL_ROWS_XPATH, filter)+ "/ancestor::table";
        AllureUtils.logActionF("Row xpath" + rowXPath);
        ElementsCollection rows = $$x(ALL_ROWS_XPATH+ "/ancestor::table");
        ...
        AllureUtils.logActionF("Row index found 2: " + index);
        if (index == -2){
...
//            String cellValue = DataContainer.resolveVariablesInText(entry.getValue());
    ...
    public void checkCell(String columnName, Map<String, String> rowFilter) {
        ...
        System.out.println("Enter into Method checkCell Line Items TABLE columnName %s rowFilter");
        String xpath = getCheckBoxElement(columnName, rowFilter);
...
//            }
            ...
            $x(xpath).scrollIntoView(true).shouldBe(visible, Duration.ofSeconds(30)).hover().click();
        }
    }
    ...
    private String getCheckBoxElement(String columnName, Map<String, String> rowFilter) {
        ...
        switch (columnName) {
            case "Import BOM":
                xpath = "//*[@data-columnid='itemsTabBomColumn']/*";
                break;
            case "Edit":
                xpath = EDIT_XPATH;
                break;
        ...
        int rowIndex = getRowIndex(rowFilter) + 1;
        return "(//table//tr[contains(@class,'x-grid-row') and not(contains(@class,'x-grid-row-summary'))])[" + rowIndex + "]" + xpath;
    }
